...
export class MainController {
  ...
  private update(): void {
    ...
    const priceFilter = <HTMLElement>document.querySelector('.filter-range-price');
    const stockFilter = <HTMLElement>document.querySelector('.filter-range-stock');
    const rangePrice = <HTMLInputElement>priceFilter.querySelector('.range-min');
    const rangeStock = <HTMLInputElement>stockFilter.querySelector('.range-min');
    ...
    if(view.filter.price.min !== parseInt(rangePrice.min) || view.filter.price.max !== parseInt(rangePrice.max)) {
      qyeryString !== '?' && (qyeryString += '&');
      qyeryString += 'price=';
      qyeryString += `${view.filter.price.min}↕${view.filter.price.max}`;
    }
    
    if(view.filter.stock.min !== parseInt(rangeStock.min) || view.filter.stock.max !== parseInt(rangeStock.max)) {
      qyeryString !== '?' && (qyeryString += '&');
      qyeryString += 'stock=';
